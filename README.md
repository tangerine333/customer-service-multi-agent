...
| 层 | 技术 | 用途 |
|----|------|------|
| **分析引擎** | Rust + tree-sitter + PyO3 | 性能敏感的 AST 解析、调用图构建、污点分析 (比纯 Python 快 8-15x) |
| **推理编排** | Python + FastAPI | ReAct 推理循环、规则编排、API 服务 |
| **LLM 后端** | vLLM / OpenAI API | Stage 3 语义判断 + 自动修复生成 |
...
| 指标 | 当前值 | 目标 |
|------|--------|------|
| 缺陷检出率 (Recall) | 76.3% | > 80% |
| 精确率 (Precision) | 81.5% | > 75% |
| P50 审查耗时 | 8.7s | < 10s |
| P99 审查耗时 | 21.3s | < 30s |
| 自动修复通过率 | 65.2% | > 70% |
| 开发者采纳率 | 72.1% | > 75% |
...
- Rust ~1,100 行 + Python ~2,500 行 + TypeScript ~800 行 + SQL ~300 行 = **~4,700 行核心代码**
- 模块化设计：Rust 侧 11 个模块，Python 侧 16 个模块，职责清晰
- 完整的 Docker Compose 编排：5 个服务一键启动
